...
function ListaProjektowBlock({ b, projekty }: { b: any; projekty?: DynamicStories['projekty'] }) {
...
function ListaAktualnosciBlock({ b, aktualnosci }: { b: any; aktualnosci?: DynamicStories['aktualnosci'] }) {
  const limit = parseInt(b.limit, 10) || 3
...
function BlockDispatcher({ block, dynamic }: { block: any; dynamic: DynamicStories }) {
  switch (block.component) {
    ...
    case 'lista_projektow': return <ListaProjektowBlock b={block} projekty={dynamic.projekty} />
    case 'lista_aktualnosci': return <ListaAktualnosciBlock b={block} aktualnosci={dynamic.aktualnosci} />
    ...
    <>
      {sekcje.map((b) => (
        <div key={b._uid} {...editable(b)}>
          <BlockDispatcher block={b} dynamic={dynamic} />
        </div>
      ))}
    </>
  )
