...
/** Prefetched dynamic story lists · pass from server */
export interface DynamicStories {
  projekty?: Array<{ id: number; full_slug: string; content: ProjektContent }>
  aktualnosci?: Array<{ id: number; full_slug: string; content: AktualnoscContent }>
  zarzad?: Array<{ id: number; content: CzlonekZarzaduContent }>
  komisja?: Array<{ id: number; content: CzlonekZarzaduContent }>
}
...
function RichTextRenderer({ doc, className }: { doc: RichText | null | undefined; className?: string }) {
      ...
      <div className="flex flex-wrap gap-3">
        {values.map((v, idx) => (
          <AnimatedSection key={v} animation="zoom-in" delay={idx * 60}>
            <span className="inline-block px-4 py-2 bg-[var(--color-ok-bg-tertiary)] text-[var(--color-ok-primary)] rounded-full text-sm font-medium border border-[var(--color-ok-border-default)] hover:scale-110 hover:bg-[var(--color-ok-primary-50)] transition-all cursor-default">{v}</span>
          </AnimatedSection>
        ))}
      </div>
    </section>
...
function HistoriaTimelineBlock({ b }: { b: any }) {
  ...
  return (
    <section className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-16">
      ...
      <AnimatedSection animation="fade-up" delay={150}><HistoryTimeline events={events} /></AnimatedSection>
    </section>
  )
}
          ...
          <div key={g._uid}>
            ...
            <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {(g.partnerzy || []).map((p: any) => (
                <li key={p._uid}>
                  {p.url ? (
                    <a href={p.url} target="_blank" rel="noopener noreferrer" className="text-[var(--color-ok-primary)] hover:underline">{p.nazwa}</a>
                  ) : (
  ...
  return (
    <section className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-12">
      <div className="bg-[var(--color-ok-bg-tertiary)] border-2 border-[var(--color-ok-gold)] rounded-2xl p-8 lg:p-10">
        ...
        {b.krs && <div className="max-w-lg"><KrsClickToCopy krs={b.krs} /></div>}
        {kroki.length > 0 && b.instrukcja_summary && (
          <details className="mt-6 group">
            <summary className="cursor-pointer text-[var(--color-ok-primary)] font-medium hover:underline list-none flex items-center gap-2">
              <ArrowRight className="size-4 group-open:rotate-90 transition-transform" />{b.instrukcja_summary}
            </summary>
...
function KartyDonacjaBlock({ b }: { b: any }) {
  ...
  return (
    <section className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-12">
      {b.tytul && <AnimatedSection animation="fade-up"><h2 className="font-headline text-3xl font-bold mb-6">{b.tytul}</h2></AnimatedSection>}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {(b.karty || []).map((k: any, i: number) => (
          <AnimatedSection key={k._uid} animation="fade-up" delay={i * 120}>
            <a href={k.cta_url} target="_blank" rel="noopener noreferrer" className="card-hover group flex flex-col h-full bg-white rounded-xl p-8 border border-[var(--color-ok-border-default)] relative overflow-hidden">
              {k.badge && <span className="inline-block self-start px-2.5 py-1 rounded-full text-xs font-semibold uppercase tracking-wider mb-4" style={{ backgroundColor: k.badge_kolor_bg || '#E8E4D9', color: k.badge_kolor_text || '#4A4A4A' }}>{k.badge}</span>}
              ...
              <span className="inline-flex items-center gap-2 font-semibold" style={{ color: k.accent_kolor || '#000' }}>{k.cta_label} <ExternalLink className="size-4 group-hover:translate-x-1 transition-transform" /></span>
            </a>
          </AnimatedSection>
        ))}
      </div>
    ...
    <section className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="bg-[var(--color-ok-bg-tertiary)] rounded-xl p-8 border-l-4 border-[var(--color-ok-primary)]">
        ...
        {b.cta_label && b.cta_tel_e164 && (
          <a href={`tel:${b.cta_tel_e164}`} className="inline-flex items-center gap-2 px-6 py-3 bg-[var(--color-ok-primary)] text-white rounded-md font-medium hover:bg-[var(--color-ok-primary-hover)] transition-colors">{b.cta_label}: {b.cta_tel}</a>
        )}
      </div>
    </section>
  )
  ...
  return (
    <section className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-12 mb-16">
      <div className="bg-[var(--color-ok-info)]/5 border border-[var(--color-ok-info)]/20 rounded-xl p-6 text-sm text-[var(--color-ok-text-secondary)]">
        <p className="leading-relaxed">{b.naglowek && <strong className="text-[var(--color-ok-text-primary)]">{b.naglowek}</strong>}{' '}{tresc}</p>
      </div>
    </section>
  )
}
    ...
    <section className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-16">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div>
          ...
          <ul className="space-y-5">
            {(b.col1_items || []).map((it: any) => (
              <li key={it._uid} className="flex items-start gap-4">
                <span className="shrink-0 size-10 rounded-full bg-[var(--color-ok-primary-50)] flex items-center justify-center text-[var(--color-ok-primary)]">{renderIcon(it.ikona)}</span>
                <div>
                  {it.etykieta && <p className="font-semibold mb-1">{it.etykieta}</p>}
                  {it.link_href ? (
                    <a href={it.link_href} className={cn('text-[var(--color-ok-primary)] hover:underline', it.ikona === 'mail' ? 'break-all' : 'font-mono text-lg')}>{it.wartosc}</a>
                  ) : (
                    <p className="text-[var(--color-ok-text-secondary)] leading-relaxed">{it.wartosc}{it.wartosc_2 && <><br />{it.wartosc_2}</>}</p>
                  )}
                </div>
              </li>
            ))}
          ...
          {b.col1_social_tytul && (
            <>
              ...
              <div className="flex flex-wrap gap-3">
                {b.col1_facebook_url && <a href={b.col1_facebook_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2.5 bg-[var(--color-ok-bg-tertiary)] text-[var(--color-ok-text-primary)] rounded-md hover:bg-[var(--color-ok-primary-50)] transition-colors"><Facebook className="size-4" /> Facebook</a>}
                {b.col1_instagram_url && <a href={b.col1_instagram_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2.5 bg-[var(--color-ok-bg-tertiary)] text-[var(--color-ok-text-primary)] rounded-md hover:bg-[var(--color-ok-primary-50)] transition-colors"><Instagram className="size-4" /> Instagram</a>}
              </div>
            </>
          )}
        </div>
        <div>
          ...
          {mapsQuery && <a href={`https://www.google.com/maps/search/?api=1&query=${mapsQuery}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-5 py-3 border-2 border-[var(--color-ok-primary)] text-[var(--color-ok-primary)] rounded-md font-medium hover:bg-[var(--color-ok-primary-50)] transition-colors"><MapPin className="size-4" /> {b.col2_maps_label || 'Otwórz w Google Maps'}</a>}
        </div>
      </div>
    </section>
  )
...
function MapaLinkBlock({ b }: { b: any }) {
  ...
  return (
    <section className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 pb-8">
      <a href={`https://www.google.com/maps/search/?api=1&query=${q}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-5 py-3 border-2 border-[var(--color-ok-primary)] text-[var(--color-ok-primary)] rounded-md font-medium hover:bg-[var(--color-ok-primary-50)] transition-colors">
...
// Dynamic blocks — use PREFETCHED data from props
function ListaProjektowBlock({ b, projekty, aboveFold = false }: { b: any; projekty?: DynamicStories['projekty']; aboveFold?: boolean }) {
  const limit = parseInt(b.limit, 10) || 3
  const stories = (projekty || []).slice(0, limit)
  return (
    <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 lg:py-20">
      <AnimatedSection animation="fade-up">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10">
          ...
          {b.link_wszystkie_label && <Link href="/projekty" className="link-underline inline-flex items-center gap-2 text-[var(--color-ok-primary)] font-medium">{b.link_wszystkie_label} <ArrowRight className="size-4" /></Link>}
        </div>
      </AnimatedSection>
            ...
            <Link href={`/${s.full_slug}`} className="card-hover group flex flex-col h-full bg-white rounded-xl overflow-hidden border border-[var(--color-ok-border-default)] hover:border-[var(--color-ok-primary)]">
              ...
              <div className="relative aspect-[16/10] overflow-hidden bg-[var(--color-ok-bg-tertiary)]">
                {s.content.zdjecie_hero?.filename ? (
                  <Image src={assetUrl(s.content.zdjecie_hero, 800)} alt={s.content.zdjecie_hero?.alt || s.content.tytul} fill sizes="(min-width: 1024px) 33vw, (min-width: 768px) 50vw, 100vw" loading={aboveFold && i < 3 ? 'eager' : 'lazy'} fetchPriority={aboveFold && i === 0 ? 'high' : undefined} className="object-cover group-hover:scale-105 transition-transform duration-500" />
                ) : (
...
function ListaAktualnosciBlock({ b, aktualnosci, aboveFold = false }: { b: any; aktualnosci?: DynamicStories['aktualnosci']; aboveFold?: boolean }) {
  const limit = parseInt(b.limit, 10) || 3
  const stories = (aktualnosci || []).slice(0, limit)
  const fmt = (d: string) => d ? new Date(d).toLocaleDateString('pl-PL', { year: 'numeric', month: 'long', day: 'numeric' }) : ''
  return (
    <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 lg:py-20">
      <AnimatedSection animation="fade-up">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10">
          {b.tytul && <h2 className="font-headline text-3xl md:text-4xl lg:text-5xl font-bold">{b.tytul}</h2>}
          <Link href="/aktualnosci" className="link-underline inline-flex items-center gap-2 text-[var(--color-ok-primary)] font-medium">Wszystkie aktualności <ArrowRight className="size-4" /></Link>
        </div>
      </AnimatedSection>
            ...
            <Link href={`/${s.full_slug}`} className="card-hover group flex flex-col h-full bg-white rounded-xl overflow-hidden border border-[var(--color-ok-border-default)] hover:border-[var(--color-ok-primary)]">
              ...
              <div className="relative aspect-[16/10] overflow-hidden bg-[var(--color-ok-bg-tertiary)]">
                {newsHeroAsset(s.content) ? (
                  <Image src={assetUrl(newsHeroAsset(s.content), 800)} alt={newsHeroAsset(s.content)?.alt || s.content.tytul} fill sizes="(min-width: 768px) 33vw, 100vw" loading={aboveFold && i < 3 ? 'eager' : 'lazy'} fetchPriority={aboveFold && i === 0 ? 'high' : undefined} className="object-cover group-hover:scale-105 transition-transform duration-500" />
                ) : (
...
function ListaZarzaduBlock({ b, zarzad, komisja }: { b: any; zarzad?: DynamicStories['zarzad']; komisja?: DynamicStories['komisja'] }) {
  const organ = b.organ || 'zarzad'
  const stories = organ === 'komisja_rewizyjna' ? (komisja || []) : (zarzad || [])
  return (
    <section className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-16">
      <AnimatedSection animation="fade-up">
        {b.tytul && <h2 className="font-headline text-3xl md:text-4xl font-bold mb-3">{b.tytul}</h2>}
...
function BlockDispatcher({ block, dynamic, position = 99 }: { block: any; dynamic: DynamicStories; position?: number }) {
  // Sekcja zaraz pod hero (pozycja 0-1) jest widoczna od razu: pierwszy rzad zdjec ladujemy bez czekania (LCP)
  const aboveFold = position <= 1
  switch (block.component) {
    ...
    case 'lista_projektow': return <ListaProjektowBlock b={block} projekty={dynamic.projekty} aboveFold={aboveFold} />
    case 'lista_aktualnosci': return <ListaAktualnosciBlock b={block} aktualnosci={dynamic.aktualnosci} aboveFold={aboveFold} />
    case 'lista_zarzadu': return <ListaZarzaduBlock b={block} zarzad={dynamic.zarzad} komisja={dynamic.komisja} />
    ...
    case 'przycisk_blok': return <section className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-4"><ButtonBlock b={block} /></section>
    default:
      console.warn('Unknown block:', block.component)
      return null
  }
}

export function BlocksClientRenderer({ sekcje, dynamic }: { sekcje: any[]; dynamic: DynamicStories }) {
  if (!sekcje?.length) return null
  return (
    <>
      {sekcje.map((b, idx) => (
        <div key={b._uid} {...editable(b)}>
          <BlockDispatcher block={b} dynamic={dynamic} position={idx} />
        </div>
      ))}
    </>
  )
